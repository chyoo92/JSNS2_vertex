...
class EDCN_type_wf3(nn.Module):
    def __init__(self,**kwargs):
        super(EDCN_type_wf3, self).__init__()
        ...
        self.conv2 = EdgeConv(torch.nn.Sequential(
                torch.nn.Linear(16*2, 32),
                torch.nn.ReLU(),
                torch.nn.Linear(32, 16),
                torch.nn.ReLU(),
            ), self.aggr)
        self.lin1 = torch.nn.Sequential(
                torch.nn.Linear(16*(self.depths), 256),
                torch.nn.ReLU(),
    ...
    def forward(self, data):
        ...
        edge_index = PyG.knn_graph(pos, self.k, batch=batch)
        ...
        comb_fea = torch.cat([x1,x2,x3],dim=1)
