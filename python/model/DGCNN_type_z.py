...
class DGCNN_type_z(nn.Module):
    def __init__(self,**kwargs):
        super(DGCNN_type_z, self).__init__()
        ...
        self.conv1 = DynamicEdgeConv(torch.nn.Sequential(
                torch.nn.Linear((self.fea + 1) * 2, 64),
                torch.nn.LeakyReLU(),
        ...
        self.lin1 = torch.nn.Sequential(
                torch.nn.Linear(64*(self.depths) + (self.fea + 1), 512),
                torch.nn.LeakyReLU(),
    ...
    def forward(self, data):
        ...
        if self.fea == 1:
            xx = torch.cat([x,pos[:,2].reshape(-1,1)],dim=1)
