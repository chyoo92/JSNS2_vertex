...
class DGCNN_type_xy(nn.Module):
    def __init__(self,**kwargs):
        super(DGCNN_type_xy, self).__init__()
        ...
        self.conv1 = DynamicEdgeConv(torch.nn.Sequential(
                torch.nn.Linear((self.fea + 2) * 2, 64),
                torch.nn.LeakyReLU(),
        ...
        self.lin1 = torch.nn.Sequential(
                torch.nn.Linear(64*(self.depths) + (self.fea + 2), 512),
                torch.nn.LeakyReLU(),
    ...
    def forward(self, data):
        ...
        if self.fea == 1:
            xx = torch.cat([x,pos[:,0:2].reshape(-1,2)],dim=1)
