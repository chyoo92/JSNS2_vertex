...
class GNN963layer(nn.Module):
    def __init__(self,**kwargs):
        super(GNN963layer, self).__init__()
        self.fea = kwargs['fea']
        self.cla = kwargs['cla']
...
#         self.conv2 = GCNConv(32, 64)
        ...
        self.fc = nn.Sequential(
            nn.Linear( 96*4, 256), nn.ReLU(), nn.BatchNorm1d(256), nn.Dropout(0.5),
    ...
    def forward(self, data):
        x = torch.cat([data.x.reshape(-1,96),data.pos.reshape(-1,96*3)],dim=1)
        out = self.fc(x)
        return out
