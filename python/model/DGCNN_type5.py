...
class DGCNN_type5(nn.Module):
    def __init__(self,**kwargs):
        super(DGCNN_type5, self).__init__()
        ...
        self.lin1 = torch.nn.Sequential(
                torch.nn.Linear(32*(self.depths) + 32*(self.depths) + 32*(self.depths) + (self.fea + 3 + self.fea + self.fea), 512),
                torch.nn.LeakyReLU(),
    ...
    def forward(self, data):
        ...
        if self.fea == 1:
            xx1 = torch.cat([x,pos[:,0].reshape(-1,1)],dim=1)
            xx2 = torch.cat([x,pos[:,1].reshape(-1,1)],dim=1)
            xx3 = torch.cat([x,pos[:,2].reshape(-1,1)],dim=1)
        elif self.fea == 2:
            xx = torch.cat([tq,x,pos],dim=1)

        x11 = self.conv12(xx1, batch)
        x21 = self.conv12(xx2, batch)
        x31 = self.conv12(xx3, batch)

        comb_fea1 = torch.cat([xx1, x11],dim=1)
        comb_fea2 = torch.cat([xx2, x21],dim=1)
        comb_fea3 = torch.cat([xx3, x31],dim=1)
        ...
        for i in range(self.depths):
            ...
            if i == 0:
                ...
                globals()['x3%s' % (i+1)] = self.conv2(x31, batch)
                comb_fea3 = torch.cat([comb_fea3,globals()['x3%s' % (i+1)]],dim=1)
                ...
                globals()['x3%s' % (i+1)] = self.conv2(globals()['x3%s' % (i)], batch)
                comb_fea3 = torch.cat([comb_fea3,globals()['x3%s' % (i+1)]],dim=1)



        comb_fea = torch.cat([comb_fea1, comb_fea2, comb_fea3],dim=1)
