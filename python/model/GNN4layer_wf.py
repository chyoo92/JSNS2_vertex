...
class GNN4layer_wf(nn.Module):
    def __init__(self,**kwargs):
        super(GNN4layer_wf, self).__init__()
        self.fea = kwargs['fea']
        self.cla = kwargs['cla']
        
      
        self.conv1 = PointConvNet(MLP([self.fea+3, 128, 256]))
        self.conv2 = PointConvNet2(MLP([256+3, 256, 512]))
        self.conv3 = PointConvNet2(MLP([512+3, 512, 512]))
        self.conv4 = PointConvNet2(MLP([512+3, 512, 512]))
        self.pool = PoolingNet(MLP([512+3, 512]))

        self.fc = nn.Sequential(
#             nn.Linear( 1024, 512), nn.ReLU(), nn.BatchNorm1d(512), nn.Dropout(0.5),
#             nn.Linear( 256, 128), nn.ReLU(), nn.BatchNorm1d(125), nn.Dropout(0.5),
