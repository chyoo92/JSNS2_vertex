...
class DGCNN_type2(nn.Module):
    def __init__(self,**kwargs):
        super(DGCNN_type2, self).__init__()
        ...
        self.conv1 = DynamicEdgeConv(torch.nn.Sequential(
                torch.nn.Linear((self.fea + 3) * 2, 512),
                torch.nn.LeakyReLU(),
    ...
    def forward(self, data):
        ...
        edge_index = knn_graph(xx,k = self.k,batch = batch)
        ...
        out = self.lin1(torch.cat([xx, x1, x2, x3, x4], dim=1))
