...
class vertexdataset_mc5(PyGDataset):
    def __init__(self, **kwargs):
        super(vertexdataset_mc5, self).__init__(None, transform=None, pre_transform=None)
        self.isLoaded = False
...
#             self.featureList.append(np.concatenate((np.array(f)[:,17:17+len(ff)].reshape(-1,len(ff),1),np.array(pos).reshape(-1,len(ff),3)),axis=2).tolist())
            ...
            for j in range(nEvent):
                ...
                new2_charge = np.concatenate([new_charge[:5],new_charge[-5:]])
                new2_pos = np.concatenate([new_pos[:5],new_pos[-5:]])
