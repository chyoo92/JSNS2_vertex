...
class vertexdataset_mc10(PyGDataset):
    def __init__(self, **kwargs):
        super(vertexdataset_mc10, self).__init__(None, transform=None, pre_transform=None)
        self.isLoaded = False
...
#                 charge.append((np.array(f)[:,17:17+len(ff)][j]/np.max(np.array(f)[:,17:17+len(ff)][j])).reshape(-1,len(ff),1))

#                 vtx.append(np.array(f)[:,10:13][j])
                pos.append(np.array(ff))

            self.chargeList.append((np.array(f)[:,17:17+len(ff)]/(np.repeat(np.max(np.array(f)[:,17:17+len(ff)],axis=1),len(ff)).reshape(-1,len(ff)))).reshape(-1,len(ff),1).tolist())
