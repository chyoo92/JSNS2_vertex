...
class vertexdataset(PyGDataset):
    ...
    def get(self, idx):
        ...
        wf = torch.Tensor(self.wfList[fileIdx][idx])
        
        
        
        data = PyGData(x = wf, pos = pos, y = vertex)
    ...
    def initialize(self, geo, itype):
        ...
        for i, fName in enumerate(self.sampleInfo['fileName']):
            ...
            for j in range(nEvent):
                ...
                if itype == 1:
                    wflist.append(f['high'][j])
                elif itype == 2:
                    wflist.append(f['low'][j])
                elif itype == 3:
                    wflist.append(f['low'][j]+f['high'][j])
