...
#             if not fName.endswith(".h5"): continue
    ...
    def initialize(self, geo, itype, tev):
        ...
        SI = self.sampleInfo
        if tev == 1:
            SI.to_csv(self.output + '/training_sampleInfo.csv')
        else:
            SI.to_csv(self.output + '/evaluation_sampleInfo.csv')
