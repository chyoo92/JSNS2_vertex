...
class vertexdataset_mc(PyGDataset):
    def __init__(self, **kwargs):
        super(vertexdataset_mc, self).__init__(None, transform=None, pre_transform=None)
        self.isLoaded = False
...
#             charge.append(np.array(f)[:,17:17+len(ff)][j])
#             vtx.append(np.array(f)[:,10:13][j])
            ...
            self.chargeList.append(np.array(f)[:,17:17+len(ff)].reshape(-1,len(ff),1).tolist())
