...
class vertexdataset_mc6(PyGDataset):
    def __init__(self, **kwargs):
        super(vertexdataset_mc6, self).__init__(None, transform=None, pre_transform=None)
        self.isLoaded = False
...
#             self.featureList.append(np.concatenate((np.array(f)[:,17:17+len(ff)].reshape(-1,len(ff),1),np.array(pos).reshape(-1,len(ff),3)),axis=2).tolist())
            ...
            for j in range(nEvent):
                ...
                new_charge = pmt_charge[j][s]

                new_pos = np.array(ff)[s]/1000

                new_charge = new_charge[new_charge>0]
                new_pos = new_pos[-len(new_charge):]
   
                new2_charge = np.concatenate([new_charge[:10],new_charge[-10:]])
                new2_pos = np.concatenate([new_pos[:10],new_pos[-10:]])
                if len(new2_charge) < 20:
                    new2_charge = new_charge
                    new2_pos = new_pos
