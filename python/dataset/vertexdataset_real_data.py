...
class vertexdataset_real_data(PyGDataset):
    def __init__(self, **kwargs):
        super(vertexdataset_real_data, self).__init__(None, transform=None, pre_transform=None)
        self.isLoaded = False
    ...
    def initialize(self,geo):
        ...
        for i, fName in enumerate(self.sampleInfo['fileName']):
            ...
            for j in range(nEvent):
                ...
                charge.append(f['pmt_charge'][j])
#                 if len(f.keys()) > 4:
#                     vtx.append(f['real_vtx'][j])
#                 else:
                vtx.append(f['jade_vtx'][j])
